...
distinct_colors = ['#e6194b', '#3cb44b', '#ffe119', '#0082c8', '#f58231', '#911eb4', '#46f0f0', '#f032e6',
                   '#d2f53c', '#fabebe', '#008080', '#000080', '#aa6e28', '#fffac8', '#2000000', '#aaffc3', '#8020000',
...
def parse_annotation2(filename):
    ...
    # with open(os.path.join(sani, filename)) as f:
        ...
        if (boxes.__len__() == 0):
            ...
            labels.append(test_labels_map['nonperson'])
            difficulties.append(0)



    return {'boxes': boxes, 'labels': labels, 'difficulties': difficulties},h1
...
def create_data_lists(kaist_path, output_folder):
    ...
    # # for path in sani:
    # sanitest_rgb2 = '/home/fereshteh/kaist/sanitized'
    # sanitest_lw2 = '/home/fereshteh/kaist/sanitized_lw'
    # sanitest12 = '/home/fereshteh/kaist/sanitized_txt/'
    # i = 0
    # for filename3 in sorted(os.listdir('/home/fereshteh/kaist/sanitized_txt')):
    #     # if (i < 1430):  # 1430
    #     if filename3.endswith('.txt'):
    #         filename1 = os.path.join(sanitest12, filename3)
    #
    #         # with open (path) as f:
    #         if (i < 22034):
    ...
    #         # if len(objects) == 0:
    #         #     continue
    #         filename4 = re.sub('.txt', ".png", str(filename3))
    #         txt_images_rgb2.append(os.path.join(sanitest_rgb2, filename4))
    #         txt_images_lw2.append(os.path.join(sanitest_lw2, filename4))
    #         i = i + 1
    #
    ...
    #
    # assert len(txt_objects2) == len(txt_images_rgb2) == len(txt_images_lw2)
    # #
    # # Save to file
    # with open(os.path.join(output_folder, 'ASANITRAIN_images_rgb.json'), 'w') as j:
    #     json.dump(txt_images_rgb2, j)
    # with open(os.path.join(output_folder, 'ASANITRAIN_images_lw.json'), 'w') as j:
    #     json.dump(txt_images_lw2, j)
    # # with open(os.path.join(output_folder, 'ALLSANITEST_images.json'), 'w') as j:
    #
    # with open(os.path.join(output_folder, 'ASANITRAIN_objects.json'), 'w') as j:
    #     json.dump(txt_objects2, j)
    # #
    # print('\nThere are %d sani images containing a total of %d objects. Files have been saved to %s.' % (
    #     len(txt_images_rgb2), n_objects2, os.path.abspath(output_folder)))
    #
    txt_images_rgb2 = list()
    txt_images_lw2 = list()
    txt_objects2 = list()
    ...
    # sani=os.path.join(kaist_path, 'sanitest_txt')
    # for path in sani:
    sanitest_rgb2 = '/home/fereshteh/kaist/sanitized_annotations1/sanitized'
    sanitest_lw2= '/home/fereshteh/kaist/sanitized_annotations1/sanitized_lw'
    sanitest12 = '/home/fereshteh/kaist/sanitized_annotations1/sanitized_annotations/'
    i = 0
    for filename3 in sorted(os.listdir('/home/fereshteh/kaist/sanitized_annotations1/sanitized_annotations')):
        # if (i > 6098):  # 1430
        if filename3.endswith('.txt'):
            filename1 = os.path.join(sanitest12, filename3)

            # with open (path) as f:
            objects2 = parse_annotation_train(filename1)
            txt_objects2.append(objects2)
            n_objects2 += len(objects2)
            # if len(objects) == 0:
            #     continue
    ...
    #
    # Save to file
    with open(os.path.join(output_folder, 'SANITRAIN1_images_rgb.json'), 'w') as j:
        json.dump(txt_images_rgb2, j)
    with open(os.path.join(output_folder, 'SANITRAIN1_images_lw.json'), 'w') as j:
        json.dump(txt_images_lw2, j)
    # with open(os.path.join(output_folder, 'ALLSANITEST_images.json'), 'w') as j:
    ...
    # # # sani=os.path.join(kaist_path, 'sanitest_txt')
    # # # for path in sani:
    sanitest_rgb1 = '/home/fereshteh/kaist/sanitest'
    sanitest_lw1 = '/home/fereshteh/kaist/sanitest_lw'
    sanitest11 = '/home/fereshteh/kaist/annotations_KAIST_testset1/annotations_KAIST_test_set/'
    i = 0
    line1=0
    for filename2 in sorted(os.listdir('/home/fereshteh/kaist/annotations_KAIST_testset1/annotations_KAIST_test_set')):
        i = i + 1
        # if (i <= 1456):  # 1430

        if filename2.endswith('.txt'):
            filename1 = os.path.join(sanitest11, filename2)

            # with open (path) as f:
            objects1,line = parse_annotation2(filename1)
            txt_objects1.append(objects1)
            # print( len(objects1))
            n_objects1 += len(objects1)
            # if len(objects) == 0:
            #     continue
            filename3 = re.sub('.txt', ".png", str(filename2))
            txt_images_rgb1.append(os.path.join(sanitest_rgb1, filename3))
            txt_images_lw1.append(os.path.join(sanitest_lw1, filename3))
            line1=line1+line
    print(line1)
    print(n_objects1)
    # for id in range(0, 2252):  # (15001,17913):
    ...
    #
    #     txt_images.append(os.path.join(kaist_path, 'sanitest', 'I' + '{0:05}'.format(id) + '.png'))
    print(len(txt_objects1), len(txt_images_rgb1) , len(txt_images_lw1))
    assert len(txt_objects1) == len(txt_images_rgb1) == len(txt_images_lw1)
    #
    # Save to file
    with open(os.path.join(output_folder, 'SANITEST1_images_rgb.json'), 'w') as j:
        json.dump(txt_images_rgb1, j)
    with open(os.path.join(output_folder, 'SANITEST1_images_lw.json'), 'w') as j:
        json.dump(txt_images_lw1, j)
    # with open(os.path.join(output_folder, 'ALLSANITEST_images.json'), 'w') as j:

    with open(os.path.join(output_folder, 'SANITEST1_objects.json'), 'w') as j:
        json.dump(txt_objects1, j)
    #
    print('\nThere are %d sani images containing a total of %d objects. Files have been saved to %s.' % (
        len(txt_images_rgb1), n_objects1, os.path.abspath(output_folder)))

    # txt_images_rgb3 = list()
    # txt_images_lw3 = list()
    # txt_objects3 = list()
    # n_objects3= 0
    # sanitest3_rgb = '/home/fereshteh/kaist/sanitest'
    # sanitest3_lw = '/home/fereshteh/kaist/sanitest_lw'
    # sanitest3 = '/home/fereshteh/kaist/test'
    # sanitest33 = '/home/fereshteh/kaist/test/annotations'
    # sanitest11 = '/home/fereshteh/MSDS-RCNN-master/annotations_KAIST_testset/annotations_KAIST_test_set/'
    # i = 1
    # with open('/home/fereshteh/kaist/test/imageSets/test-all-20.txt') as f:
    #     ids = f.read().splitlines()
    #
    # for filename5 in ids:
    #     k=int(filename5[12:])
    #     k=k+1
    #     if (k%20==0):
    #     # if (i < 1430):  # 1430
    #     # if filename2.endswith('.txt'):
    #         if (k%20==0):
    #             filename51 = re.sub('/', "_", str(filename5))
    #             filename1 = os.path.join(sanitest11, filename51 + '.txt')
    #         else:
    #             filename1 = os.path.join(sanitest33, filename5+'.txt')
    #
    #
    #         # with open (path) as f:
    #         objects3 = parse_annotation1(filename1)
    #         txt_objects3.append(objects3)
    #         n_objects3 += len(objects3)
    #         # if len(objects) == 0:
    #         #     continue
    #         # filename3 = re.sub('.txt', ".png", str(filename2))
    #         filename3=re.sub('/I', "/visible/I", str(filename5))
    #         filename4=re.sub('/I', "/lwir/I", str(filename5))
    #         txt_images_rgb3.append(os.path.join(sanitest3, filename3+".png"))
    #         txt_images_lw3.append(os.path.join(sanitest3, filename4+".png"))
    #     i = i + 1
    #
    # # for id in range(0, 2252):  # (15001,17913):
    # #     # Parse annotation's XML file
    ...
    # #     txt_images.append(os.path.join(kaist_path, 'sanitest', 'I' + '{0:05}'.format(id) + '.png'))
    #
    # assert len(txt_objects3) == len(txt_images_rgb3) == len(txt_images_lw3)
    # #
    # # Save to file
    # with open(os.path.join(output_folder, 'VIS_SANITEST_images_rgb.json'), 'w') as j:
    #     json.dump(txt_images_rgb3, j)
    # with open(os.path.join(output_folder, 'VIS_SANITEST_images_lw.json'), 'w') as j:
    #     json.dump(txt_images_lw3, j)
    # # with open(os.path.join(output_folder, 'ALLSANITEST_images.json'), 'w') as j:
    #
    # with open(os.path.join(output_folder, 'VIS_SANITEST_objects.json'), 'w') as j:
    #     json.dump(txt_objects3, j)
    # #
    # print('\nThere are %d sani images containing a total of %d objects. Files have been saved to %s.' % (
    #     len(txt_images_rgb3), n_objects3, os.path.abspath(output_folder)))
    #
    ...
    sanitest1 = '/home/fereshteh/kaist/annotations_KAIST_testset1/annotations_KAIST_test_set/'
    ...
    for filename in sorted(os.listdir('/home/fereshteh/kaist/annotations_KAIST_testset1/annotations_KAIST_test_set/')):
        ...
        if filename.endswith('.txt'):
            ...
            # with open (path) as f:
            objects = parse_annotation_val(filename1)
            txt_objects.append(objects)
            n_objects += len(objects)
            # if len(objects) == 0:
            #     continue
            filename2 = re.sub('.txt', ".png", str(filename))
            txt_images_rgb.append(os.path.join(sanitest_rgb, filename2))
            txt_images_lw.append(os.path.join(sanitest_lw, filename2))
            i = i + 1
    ...
    assert len(txt_objects) == len(txt_images_rgb) == len(txt_images_lw)
    #
    # Save to file
    with open(os.path.join(output_folder, 'SANIVAL_images_rgb.json'), 'w') as j:
        json.dump(txt_images_rgb, j)
    with open(os.path.join(output_folder, 'SANIVAL_images_lw.json'), 'w') as j:
    ...
    #
    print('\nThere are %d sani images containing a total of %d objects. Files have been saved to %s.' % (
        len(txt_images_rgb), n_objects, os.path.abspath(output_folder)))
...
def calculate_mAP1(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    ...
    for c in range(1, n_classes):
        ...
        for i in range(0, n_class_objects1):
            if ((true_class_boxes1[i][3] - true_class_boxes1[i][1]) > scale1):
        ...
        # det_class_scores1=det_class_scores1.unsqueeze(1)
        for i in range(0, n_class_detections1):
            if ((det_class_boxes1[i][3] - det_class_boxes1[i][1] > scale1)):
        ...
        true_positives = torch.zeros((n_class_detections2), dtype=torch.float).to(device)  # (n_class_detections)
        false_positives = torch.zeros((n_class_detections2), dtype=torch.float).to(device)  # (n_class_detections)
        ...
        precision[c - 1] =true_positives.sum() / (
                true_positives.sum() + false_positives.sum() + 1e-10)
        recall[c - 1] = true_positives.sum() / n_easy_class_objects
        true_positives[c - 1] = true_positives.sum()
        false_positives[c - 1] = false_positives.sum()
...
def calculate_mAP10(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    ...
    for c in range(1, n_classes):
        ...
        if det_class_images2.size(0) == 0:
            # continue torch.Tensor()
            det_class_boxes2 = torch.Tensor()  # (n_detections, 4)
            det_class_scores2 = torch.Tensor()  # (n_detections)
        else:
            det_class_boxes2 = torch.cat(det_class_boxes2, dim=0)  # (n_detections, 4)
            det_class_scores2 = torch.cat(det_class_scores2, dim=0)  # (n_detections)
...
def calculate_mAP8(h,bir,det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    ...
    for c in range(1, n_classes):
        ...
        if true_class_boxes.size(0) != 0:
            ...
            for i in range(object_boxes_d.size(0)):
                ...
                cv2.rectangle(image, (int(box_location[0]), int(box_location[1])),
                                      (int(box_location[2]), int(box_location[3])), (0, 0, 0), 1)
        #
        ...
        #                               (int(box_location1[2]), int(box_location1[3])),(0, 120, 255),
        #                               cv2.FILLED)
        #                 image=cv2.addWeighted(image1,0.4,image,0.6,0)
        #                 cv2.putText(image, det_scores,
        #                             org=(int(box_location1[0]) + 2, int(box_location1[1]) - 4),
        #                             fontFace=font, fontScale=1 / 2, color=(0, 120, 255), thickness=2,
        #                             lineType=cv2.LINE_AA)
        #                 continue
        #
        #         # Find maximum overlap of this detection with objects in this image of this class
        ...
        #                                     cv2.putText(image, det_scores,
        #                                                 org=(int(box_location1[0]) + 2, int(box_location1[1]) - 4),
        #                                                 fontFace=font, fontScale=1 / 2, color=(0, 120, 255),
        #                                                 thickness=2,
        #                                                 lineType=cv2.LINE_AA)
        #                             else:
        #                                 if ((object_boxes5[ind5][2] - object_boxes5[ind5][0]) / (
        ...
        # #                                       (int(box_location1[2]), int(box_location1[3])), (255, 130, 0),
        # #                                       3)
        # #                 cv2.putText(image, det_scores,
        # #                                     org=(int(box_location1[0]) + 2, int(box_location1[1]) - 4),
        # #                                     fontFace=font, fontScale=1 / 2, color=(255, 130, 0), thickness=2,
        # #                                     lineType=cv2.LINE_AA)
        # #                 true_class_boxes_detected[original_ind] = 1  # this object has now been detected/accounted for
        # #
        # #         else:
    ...
    cv2.imwrite('/home/fereshteh/MSDS-RCNN-master/test/' + str(h) + '.png', image)
...
def expand(image_rgb, image_lw, boxes, filler):
    """
    ...
    new_image_rgb = torch.ones((3, new_h, new_w), dtype=torch.float) * filler.unsqueeze(1).unsqueeze(
        1)  # (3, new_h, new_w)
    new_image_lw = torch.ones((3, new_h, new_w), dtype=torch.float) * filler.unsqueeze(1).unsqueeze(
        1)  # (3, new_h, new_w)
    ...
    new_image_rgb[:, top:bottom, left:right] = image_rgb
    new_image_lw[:, top:bottom, left:right] = image_lw
    # Adjust bounding boxes' coordinates accordingly

    new_boxes = boxes + torch.FloatTensor([left, top, left, top]).unsqueeze(
        0).float()  # (n_objects, 4), n_objects is the no. of objects in this image

    return new_image_rgb, new_image_lw, new_boxes


def random_crop(image_rgb, image_lw, boxes, labels, difficulties):
    """
    ...
    while True:
        ...
        if min_overlap is None:
            return image_rgb, image_lw, boxes, labels, difficulties
        ...
        for _ in range(max_trials):
            ...
            new_image_lw = image_lw[:, top:bottom, left:right]  # (3, new_h, new_w)
            ...
            return new_image_rgb, new_image_lw, new_boxes, new_labels, new_difficulties


def flip(image_rgb, image_lw, boxes):
    """
    ...
    new_image_lw = FT.hflip(image_lw)
    ...
    new_boxes[:, 1] = boxes[:, 1]
    new_boxes[:, 3] = boxes[:, 3]
    new_boxes[:, 0] = image_rgb.width - boxes[:, 0]

    new_boxes[:, 2] = image_rgb.width - boxes[:, 2]
...
def resize(image_rgb, image_lw, boxes, dims=(513, 513), return_percent_coords=True):
    """
    ...
    new_image_lw = FT.resize(image_lw, dims)
...
def transform(m,image_rgb, image_lw, boxes, labels, difficulties, split):
    """
    ...
    """
    assert split in {'NSANITEST','NNSANITEST','DDSANITEST','SANITEST','SANITEST1','SANITRAIN','SANITRAIN1','SANITRAIN2','SANIVAL',"VIS_SANITEST"}
    ...
    new_image_lw = image_lw
    new_boxes = boxes
    new_labels = labels
    new_difficulties = difficulties
    # Skip the following operations if validation/evaluation
    if ( (split == 'SANITRAIN' or split=='SANITRAIN1')):
        # A series of photometric distortions in random order, each with 50% chance of occurrence, as in Caffe repo
        # if new_image_rgb.size ==(640,512):

        new_image_rgb = photometric_distort(new_image_rgb)

        # Convert PIL image to Torch tensor

        new_image_lw = photometric_distort(new_image_lw)

        # Convert PIL image to Torch tensor
        new_image_lw = FT.to_tensor(new_image_lw)
        new_image_rgb = FT.to_tensor(new_image_rgb)
        # Expand image (zoom out) with a 50% chance - helpful for training detection of small objects
        # Fill surrounding space with the mean of ImageNet data that our base VGG was trained on
        if random.random() < 0.5:
            new_image_rgb, new_image_lw, new_boxes = expand(new_image_rgb, new_image_lw, boxes, filler=mean)
        ...
        new_image_rgb, new_image_lw, new_boxes, new_labels, new_difficulties = random_crop(new_image_rgb, new_image_lw,
                                                                                           new_boxes, new_labels,
                                                                                           new_difficulties)

        new_image_rgb = FT.to_pil_image(new_image_rgb)
        new_image_lw = FT.to_pil_image(new_image_lw)

        # Flip image with a 50% chance
        if random.random() < 0.5:
            new_image_rgb, new_image_lw, new_boxes = flip(new_image_rgb, new_image_lw, new_boxes)
            # if ((new_boxes[:, 0] < 0).sum()) > 0:
            #     print("jj")
            # for i in range(0, len(new_boxes)):
            #     if (new_boxes[i, 2] == 0):
    ...
    new_image_rgb1, new_image_lw1, new_boxes = resize(new_image_rgb, new_image_lw, new_boxes, dims=(513, 513))
    # if ((new_boxes[:,0]<0).sum())>0:
    #     print("jj")
    # for i in range(0, len(new_boxes)):
    #     if (new_boxes[i, 2] == 0):
    ...
    #         new_boxes[i, 3] = 0
    # Convert PIL image to Torch tensor
    new_image_rgb = FT.to_tensor(new_image_rgb1)
    new_image_lw = FT.to_tensor(new_image_lw1)
    # new_image_rgb1 = FT.to_pil_image(new_image_rgb1)
    # Normalize by mean and standard deviation of ImageNet data that our base VGG was trained on
    new_image_rgb = FT.normalize(new_image_rgb, mean=mean, std=std)
    new_image_lw = FT.normalize(new_image_lw, mean=mean, std=std)
    # if ((new_boxes[:,0]<0).sum())>0:
    #     print("jj")
    # for i in range(0, len(new_boxes)):
    #     if (new_boxes[i, 2] == 0):
    ...
    #     if (new_boxes[i, 2] == 0):
    #         new_boxes[i, 3] = 0
    return new_image_rgb1,new_image_rgb, new_image_lw, new_boxes, new_labels, new_difficulties

def transform1(m,image_rgb, image_lw, boxes, labels, difficulties, split):
    """
    Apply the transformations above.
    ...
    #         new_boxes[i, 3] = 0
    return new_image_rgb1,new_image_rgb, new_image_lw, new_boxes, new_labels, new_difficulties
def transform2(m,image_rgb, image_lw, boxes, labels, difficulties, split):
    """
    Apply the transformations above.
    ...
    #         new_boxes[i, 3] = 0
    return new_image_rgb1,new_image_rgb, new_image_lw, new_boxes, new_labels, new_difficulties
def transform3(m,image_rgb, image_lw, boxes, labels, difficulties, split):
    """
    Apply the transformations above.
    ...
    #         new_boxes[i, 3] = 0
    return new_image_rgb1,new_image_rgb, new_image_lw, new_boxes, new_labels, new_difficulties
def transform4(m,image_rgb, image_lw, boxes, labels, difficulties, split):
    """
    Apply the transformations above.
...
def save_checkpoint(epoch, epochs_since_improvement, model, optimizer, loss, best_loss, is_best):
    ...
    filename = 'dec_mob2_NEW_fine_checkpoint_ssd_fusion_bn_513_3.pth.tar'
...
def detect_objects(priors_cxcy, predicted_locs_lw, predicted_locs_rgb, predicted_scores_lw, predicted_scores_rgb,predicted_scores,
                   min_score, max_overlap, top_k, n_classes):
    """
    ...
    assert n_priors == predicted_locs_lw.size(1) == predicted_scores_lw.size(1) == predicted_scores.size(1)
    ...
    for i in range(batch_size):
        ...
        for c in range(1, n_classes):
            ...
            class_decoded_locs_lw = decoded_locs_lw[score_above_min_score]  # (n_qualified, 4)
            class_decoded_locs_rgb = decoded_locs_rgb[score_above_min_score]
            ...
            class_decoded_locs_rgb = class_decoded_locs_rgb[sort_ind]  # (n_min_score, 4)
            class_decoded_locs_lw = class_decoded_locs_lw[sort_ind]
            ...
            overlap_rgb, overlap1_rgb = find_jaccard_overlap(class_decoded_locs_rgb,
                                                             class_decoded_locs_rgb)  # (n_qualified, n_min_score)
            ...
            suppress_rgb = torch.zeros((n_above_min_score), dtype=torch.bool).to(device)
            ...
            suppress_lw = torch.zeros((n_above_min_score), dtype=torch.bool).to(device)
            # Consider each box in order of decreasing scores
            ...
            image_scores_f.append(class_scores[~ suppress_rgb])
            image_scores_f.append(class_scores[~ suppress_lw])
            image_scores_f = torch.cat(image_scores_f, dim=0)
            image_boxes_f = torch.cat(image_boxes_f, dim=0)
            image_scores_f, sort_ind = image_scores_f.sort(dim=0, descending=True)
            image_boxes_f = image_boxes_f[sort_ind]  # (n_min_score, 4)

            overlap, overlap1 = find_jaccard_overlap(image_boxes_f,
                                                     image_boxes_f)
            # score_above_min_score_f = image_scores_f > min_score
            # image_scores_f = image_scores_f[score_above_min_score_f]  # (n_qualified), n_min_score <= 8732
            #
            # image_boxes_f = image_boxes_f[score_above_min_score]  # (n_qualified, 4)
            # # class_decoded_locs_rgb = decoded_locs_rgb[score_above_min_score]
            # n_above_min_score_f = score_above_min_score_f.sum().item()
            suppress = torch.zeros((len(image_scores_f)), dtype=torch.bool).to(device)  # (n_qualified)
            ...
            for box in range(image_boxes_f.size(0)):
                ...
                # if suppress[box] == 1:
                #     image_scores_f[box] = image_scores_f[box]
                # The max operation retains previously suppressed boxes, like an 'OR' operation
...
def detect_objects1(priors_cxcy, predicted_locs_lw, predicted_locs_rgb, predicted_scores_lw, predicted_scores_rgb,
                   min_score, max_overlap, top_k, n_classes):
    """
    Decipher the 8732 locations and class scores (output of ths SSD300) to detect objects.
    For each class, perform Non-Maximum Suppression (NMS) on boxes that are above a minimum threshold.
    ...
    for i in range(batch_size):
        ...
        for c in range(1, n_classes):
            ...
            for box in range(class_decoded_locs_rgb.size(0)):
                ...
                if suppress_rgb[box] == 1:
                    continue
                class_scores_rgb=class_scores_rgb-0.1
                # Suppress boxes whose overlaps (with this box) are greater than maximum overlap
                # Find such boxes and update suppress indices
                # suppress = torch.max(suppress, overlap[box] > max_overlap)
                condition_rgb = overlap_rgb[box] > max_overlap
            ...
            for box in range(class_decoded_locs_lw.size(0)):
                ...
                class_scores_lw[box]=class_scores_lw[box]+0.0
            ...
            for box in range(image_boxes_f.size(0)):
                ...
                # Find such boxes and update suppress indices
                # suppress = torch.max(suppress, overlap[box] > max_overlap)
                condition = overlap[box] >0.5
                condition = torch.tensor(condition, dtype=torch.bool).to(device)
                # image_scores_f=image_scores_f[overlap[box] > max_overlap]+10
                if (condition.sum().item()>1):
                    image_scores_f[box]=image_scores_f[box]*2
                suppress = torch.max(suppress, condition)
                # if suppress[box] == 1:
                #     image_scores_f[box] = image_scores_f[box]
                # The max operation retains previously suppressed boxes, like an 'OR' operation
        ...
        image_scores = torch.cat(image_scores, dim=0)  # (n_objects)
        n_objects = image_scores.size(0)
        # score_above_min_score = image_scores > 0.5
        # image_boxes = image_boxes[score_above_min_score] # (n_objects, 4)
        # image_labels = image_labels[score_above_min_score]  # (n_objects)
        # image_scores = image_scores[score_above_min_score]
        # Keep only the top k objects
        if n_objects > top_k:
            image_scores, sort_ind = image_scores.sort(dim=0, descending=True)
            image_scores = image_scores[:top_k]  # (top_k)
...
def detect_objects2(model, priors_cxcy, predicted_locs, predicted_scores, min_score, max_overlap, top_k, n_classes) -> object:
    """
    Decipher the 8732 locations and class scores (output of ths SSD300) to detect objects.
    For each class, perform Non-Maximum Suppression (NMS) on boxes that are above a minimum threshold.
    :param predicted_locs: predicted locations/boxes w.r.t the 8732 prior boxes, a tensor of dimensions (N, 8732, 4)
    ...
    for i in range(batch_size):
        ...
        for c in range(1, n_classes):
            ...
            # Find the overlap between predicted boxes
            overlap, overlap1 = find_jaccard_overlap(class_decoded_locs,
...
from PIL import Image, ImageEnhance
def calculate_mAP_tr(trackers,h1,bir,det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties,track_boxes):
    ...
    font = cv2.FONT_HERSHEY_SIMPLEX
    # bir = Image.open(bir, mode='r')
    # bir1 = FT.resize(bir, (800, 800))
    #
    # bir = FT.resize(bir, (512, 640))
    # image = np.ascontiguousarray(bir)
    # image2 = np.ascontiguousarray(bir)
    # image=bir
    image2=bir
    # print(bir.shape)
    # image=bir
    ...
    # det_labels = torch.cat(det_labels, dim=0)  # (n_detections)
    # det_scores = torch.cat(det_scores, dim=0)  # (n_detections)
    print(det_images.size(0) , det_boxes.size(0) , det_labels.size(0) , det_scores.size(0))
    assert det_images.size(0) == det_boxes.size(0) == det_labels.size(0) == det_scores.size(0)
    ...
    #     track_boxes1.append(torch.tensor(track_boxes[i]).unsqueeze(0))  # (n_class_detections, 4)
    #     # track_success.append(track_success[i].unsqueeze(0))
    # if len(track_boxes) == 0:
    #     # continue torch.Tensor()
    #     track_boxes1 = torch.Tensor()  # (n_detections, 4)
    #     # track_success = torch.Tensor()
    # else:
    #     track_boxes1= torch.cat(track_boxes1, dim=0)  # (n_detections, 4)
    ...
    for c in range(1, n_classes):
        ...
        original_dims = torch.FloatTensor(
            [800,
            800, 800, 800]).unsqueeze(0)
        if true_class_boxes.size(0) != 0:
            object_boxes_d = true_class_boxes.cpu() * original_dims.cpu()
            for i in range(object_boxes_d.size(0)):
                box_location = object_boxes_d[i].tolist()
                cv2.rectangle(image2, (int(box_location[0]), int(box_location[1])),
        ...
        #                 this_detection_box1 = this_detection_box.cpu() * original_dims.cpu()
        #                 box_location1 = this_detection_box1[0].tolist()
        #                 bb = np.hstack(box_location1).astype(np.int32)
        #
        #                 # bb[2]=bb[2] - bb[0]
        #                 # bb[3] = bb[3] - bb[1]
        #                 print(bb, "det")
        ...
        #                 bb1 = (bb[0], bb[1], bb[2] - bb[0], bb[3] - bb[1])
        #                 print(bb1, "det to tr")
        #                 # bb1 = np.array(bb1)
        ...
        #                 # print(t,"tr")
        #
        #                 # tracker.add(tracker, image2, bb1)
        #                 trackers.add(tracker, image2, bb1)
        #
        #                 cv2.rectangle(image2, (int(box_location1[0]), int(box_location1[1])),
        #                               (int(box_location1[2]), int(box_location1[3])),(0, 0, 255),
        #                               1)
        #                 # image1 = image.copy()
        #                 # image2=cv2.addWeighted(image1,0.4,image2,0.6,0)
        #                 cv2.putText(image2, det_scores,
        #                             org=(int(box_location1[0]) + 2, int(box_location1[1]) - 4),
        ...
        # m=torch.cat(det,dim=0)
        # image1 = image2.copy()
        # # image2=cv2.addWeighted(image1,0.4,image2,0.6,0)
        # for d in range(len(track_boxes1)):
        #     # if d<n_class_detections:
        #     if track_boxes1.nelement() != 0:
        #         this_detection_box = track_boxes1[d].unsqueeze(0)  # (1, 4)
        #         # this_image = det_class_images3[d]  # (), scalar
        ...
        #         #     max_overlap, ind = torch.max(overlaps.squeeze(0), dim=0)  # (), () - scalars
        #         #     original_ind = torch.LongTensor(range(len(true_class_boxes)))[true_class_images == this_image][ind]
        ...
        #         box_location1 = this_detection_box1[0].tolist()
        #         cv2.rectangle(image1, (int(box_location1[0]), int(box_location1[1])),
        #                               (int(box_location1[0])+int(box_location1[2]), int(box_location1[1])+int(box_location1[3])), (125, 135, 180),
        #                               -1)
        #         image2 = cv2.addWeighted(image1, 0.4, image2, 0.6, 0)
        ...
        #             #         box_location1 = this_detection_box1[0].tolist()
        #             #         cv2.rectangle(image2, (int(box_location1[0]), int(box_location1[1])),
        #             #                               (int(box_location1[2]), int(box_location1[3])), (0, 0, 0),
        #             #                               3)
        #             #         # cv2.putText(image, track_success,
        #             #         #                     org=(int(box_location1[0]) + 2, int(box_location1[1]) - 4),
        #             #         #                     fontFace=font, fontScale=1 / 2, color=(0, 0, 0), thickness=1,
        #             #         #                     lineType=cv2.LINE_AA)
    ...
    #             original_ind = torch.LongTensor(range(true_class_boxes.size(0)))[true_class_images == this_image][ind]
    #             this_detection_box1 = this_detection_box.cpu() * original_dims.cpu()
    #             if ((max_overlap.item() > 0.5) or ((int(object_boxes[ind][3])-int(object_boxes[ind][1]))<s1 & (max_overlap.item() > 0.1))):
    #
    #                 if true_class_boxes_detected[original_ind] == 0:
    #                     true_positives[d] = 1
    #                     box_location1 = this_detection_box1[0].tolist()
    ...
    cv2.imwrite('/home/fereshteh/MSDS-RCNN-master/test/' + str(h1) + '.png', image2)
...
def calculate_mAP_kaist(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    ...
    for c in range(1, n_classes):
        ...
        scale = 55 / 512
        true_class_images_LAMR=list()
        for i in range(0, n_class_objects1):


            true_class_images_LAMR.append(true_class_images1[i])  # (n_class_detections)

        true_class_images_LAMR = torch.LongTensor(true_class_images_LAMR).to(device)
        print(true_class_images_LAMR.size(0))
        LAMR=true_class_images_LAMR.size(0)
        for i in range(0, n_class_objects1):
            if ((true_class_boxes1[i][3] - true_class_boxes1[i][1]) <= scale):
        ...
        # wd=det_class_boxes1[:][1]-det_class_boxes1[:][0]
        # hd=det_class_boxes1[:][3]-det_class_boxes1[:][2]
        det_class_images_lamr=list()
        for i in range(0, n_class_detections1):
            if ((det_class_boxes1[i][3] - det_class_boxes1[i][1] > scale)):
                det_class_images_lamr.append(det_class_images1[i])  # (n_class_detections)

        det_class_images_lamr = torch.LongTensor(det_class_images_lamr).to(device)  # (n_detections)
        print(det_class_images_lamr, "lamr")
        n_class_detections_lamr = det_class_images_lamr.size(0)
        det_class_images1 = det_class_images1[det_class_scores1 >-6]  # (n_class_detections)
        det_class_boxes1 = det_class_boxes1[det_class_scores1  >-6]  # (n_class_detections, 4)
        det_class_scores1 = det_class_scores1[det_class_scores1  >-6]  # (n_class_detections)
        n_class_detections1 = det_class_boxes1.size(0)
        print(n_class_detections1, "50")
        det_class_images = list()
        det_class_boxes = list()
        det_class_scores = list()
        det_class_images1 = det_class_images1.unsqueeze(1)
        ...
        if n_class_detections2 == 0:
            # continue torch.Tensor()
            det_class_boxes2 = torch.Tensor()
            det_class_scores2 = torch.Tensor()  # (n_detections, 4)
        else:
        ...
        if n_class_detections == 0 & n_class_detections2 != 0:
            ...
            object_difficulties = true_class_difficulties[
                true_class_images == this_image6]  # (n_class_objects_in_img)
            object_labels = true_class_labels[true_class_images == this_image6]
            ...
            # If the maximum overlap is greater than the threshold of 0.5, it's a match
            if ((max_overlap6.item() > 0.5) & bool((object_labels[ind6] ==1))):
                # If the object it matched with is 'difficult', ignore it
                ...
                if (object_difficulties[ind6] != 2 ):
                    ...
                    if true_class_boxes_detected[original_ind6] == 0:
                        qq = qq + 1
            elif ((max_overlap61.item() > 0.5) & bool((object_labels[ind61] == 2))):
                print()
                qq = qq + 1
                ...
                if ((max_overlap6.item() > 0.5) & bool(object_labels6[ind6]==1)):
                    ...
                    # If this object has already not been detected, it's a true positive
                        ...
                        if true_class_boxes_detected[original_ind6] == 0:
                            ...
                            qq = qq + 1
                elif ((max_overlap61.item() > 0.5) & bool(object_labels6[ind61]==2)):
                    qq=qq+1
        # Compute cumulative precision and recall at each detection in the order of decreasing scores
        cumul_true_positives = torch.cumsum(true_positives, dim=0)  # (n_class_detections)
        cumul_false_positives = torch.cumsum(false_positives, dim=0)  # (n_class_detections)
        ...
        while j>=0:
            ...
            while i>=0:
                tmp=j/10.0
                if cumul_recall[i]<tmp:
                    stat_idx=i
                    if j>0:
                        max_precs[j-1]=max_precs[j]
                        break
                else:
                    if max_precs[j]<cumul_precision[i]:
                        max_precs[j]=cumul_precision[i]
                i=i-1
            j=j-1
        ...
        print((qq+false_positives[c-1].cpu()))
        fppi=false_positives[c - 1].cpu()/(qq+false_positives[c-1].cpu())
        mr=(1-recall[c - 1])
        ...
        # lamr = math.exp(np.mean(np.log(ref)))
        ##me
        # if (c==1):
        # precision=cumul_precision.max()
        # recall=cumul_recall.max()
        ...
        plt.ylabel('Precision' + str(c))
        plt.show()
        # plt.plot(np.logspace(-2.0,0.0,num=9), ref, color='gold', lw=2)
        #
        # plt.xlim([0.001, 1])
        # plt.ylim([0.0, 1.0])
        # plt.xlabel('Recall' + str(c))
    ...
    return average_precisions, mean_average_precision, precision, recall, f, n_easy_class_objects, true_positives, false_positives,lamr,ap
