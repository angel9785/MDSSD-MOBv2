...
class BaseNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        for end_layer_index in source_layer_indexes:
            ...
            if flag == 0:
                features_46x46 = layer_output
                flag += 1
            elif flag == 1:
                features_23x23 = layer_output

        return features_46x46, features_23x23
...
class FUSION_PredictionConvolutions(nn.Module):
    ...
    def __init__(self, n_classes, backbone_net):
        ...
        if backbone_net == 'MobileNetV2':
            ...
            # self.loc_conv23_2 = nn.Conv2d(256, n_boxes['conv23_2'] * 4, kernel_size=3, padding=1)
            ...
            # self.cl_conv23_2 = nn.Conv2d(256, n_boxes['conv23_2'] * n_classes, kernel_size=3, padding=1)
    ...
    def forward(self, conv11_feats, conv13_feats, conv14_2_feats, conv15_2_feats, conv16_2_feats):  #, conv23_2_feats,conv18_2_feats
        batch_size = conv11_feats.size(0)

        l_conv11 = self.loc_conv11(conv11_feats)  # (N, 16, 46, 46)
        l_conv11 = l_conv11.permute(0, 2, 3,
                                    1).contiguous()  # (N, 46, 46, 16), to match prior-box order (after .view())
        ...
        # l_conv23_2 = self.loc_conv23_2(conv23_2_feats)  # (N, 16, 1, 1)
        # l_conv23_2 = l_conv23_2.permute(0, 2, 3, 1).contiguous()  # (N, 1, 1, 16)
        # l_conv23_2 = l_conv23_2.view(batch_size, -1, 4)  # (N, 4, 4)
        #
        ...
        c_conv11 = self.cl_conv11(conv11_feats)  # (N, 4 * n_classes, 46, 46)
        c_conv11 = c_conv11.permute(0, 2, 3,
                                    1).contiguous()  # (N, 46, 46, 4 * n_classes), to match prior-box order (after .view())
        ...
        # c_conv23_2 = self.cl_conv23_2(conv23_2_feats)  # (N, 4 * n_classes, 1, 1)
        # c_conv23_2 = c_conv23_2.permute(0, 2, 3, 1).contiguous()  # (N, 1, 1, 4 * n_classes)
        # c_conv23_2 = c_conv23_2.view(batch_size, -1, self.n_classes)  # (N, 4, n_classes)
        ...
        locs = torch.cat([l_conv11, l_conv13, l_conv14_2, l_conv15_2, l_conv16_2],
                         dim=1)  # (N, 2268, 4) , l_conv23_2, l_conv18_2
        classes_scores = torch.cat([c_conv11, c_conv13, c_conv14_2, c_conv15_2, c_conv16_2],
                                   dim=1)  # (N, 2268, n_classes) , c_conv23_2, c_conv18_2
...
class FUSION_AuxillaryConvolutions(nn.Module):
    ...
    def forward(self, FUSION_inp_features_23x23):
        FUSION_features = []
        x = FUSION_inp_features_23x23
...
class FUSION_DeConvolutions(nn.Module):
    ...
    def __init__(self):
        ...
        self.refine23_rgb = conv_3(1280)
        self.refine46_rgb = conv_3(96)


        self.refine5_lw = conv_3(256)
        self.refine9_lw = conv_3(512)
        self.refine23_lw = conv_3(1280)
        self.refine46_lw = conv_3(96)
        ...
        self.dconv23 = dconv(1280, 96, 1, 1)
        ...
        self.late23 = late_conv_3(1280)
        self.late46 = late_conv_3(96)
        ...
        self.conv46_1 = nn.Sequential(
            nn.Conv2d(192, 96, 1, 1, 0)
            # ,#,bia_1 =False
            #     ReLU()
        )
        self.conv23_1 = nn.Sequential(
            nn.Conv2d(2560, 1280, 1, 1, 0)
            # ,
            # ReLU()
        )
    ...
    def forward(self,RGB_features_46x46,RGB_features_23x23, RGB_features_9x9, RGB_features_5x5, RGB_features_3x3, LW_features_46x46, LW_features_23x23,LW_features_9x9, LW_features_5x5, LW_features_3x3):#,RGB_features_2x2,RGB_features_1x1, LW_features_2x2, LW_features_1x1
        ...
        LW_refine1_23x23 = self.refine23_lw(LW_features_23x23)
        RGB_refine1_23x23 = self.refine23_rgb(RGB_features_23x23)
        cat_23x23 = torch.cat((RGB_features_23x23, LW_features_23x23), dim=1)
        fusion_23x23 = self.conv23(cat_23x23)
        cat_refine_23x23 = torch.cat((LW_refine1_23x23, RGB_refine1_23x23), dim=1)
        fusion_refine_23x23 = self.conv23_1(cat_refine_23x23)

        # x23 = fusion_refine_23x23 + FUSION_dconv_9x9+fusion_23x23# + RGB_refine1_23x23 + LW_refine1_23x23
        x23 =fusion_refine_23x23 + FUSION_dconv_9x9+fusion_23x23# + RGB_refine1_23x23 + LW_refine1_23x23

        FUSION_refine_23x23 = self.late23(x23)

        FUSION_dconv_23x23 = self.dconv23(FUSION_refine_23x23)

        LW_refine1_46x46 = self.refine46_lw(LW_features_46x46)
        RGB_refine1_46x46 = self.refine46_rgb(RGB_features_46x46)
        cat_46x46 = torch.cat((RGB_features_46x46, LW_features_46x46), dim=1)
        fusion_46x46 = self.conv46(cat_46x46)
        cat_refine_46x46 = torch.cat((LW_refine1_46x46, RGB_refine1_46x46), dim=1)

        fusion_refine_46x46 = self.conv46_1(cat_refine_46x46)
        # x46 = fusion_refine_46x46 + FUSION_dconv_23x23 +fusion_46x46#+ RGB_refine1_46x46 + LW_refine1_46x46
        x46 = fusion_refine_46x46 + FUSION_dconv_23x23+fusion_46x46#+ RGB_refine1_46x46 + LW_refine1_46x46

        FUSION_refine_46x46 = self.late46(x46)


        return FUSION_refine_46x46 ,FUSION_refine_23x23 ,FUSION_refine_9x9 ,FUSION_refine_5x5 , FUSION_refine_3x3#, FUSION_refine_2x2, FUSION_refine_1x1
    ...
    def forward(self, x1, x2):
        ...
        if self.backbone_net == 'MobileNetV2':
            for index, feat in enumerate(self.RGB_base_net.features):
                x1 = feat(x1)
                if index == 13:
                    RGB_features_46x46 = x1
                if index == 18:
                    RGB_features_23x23 = x1

            for index1, feat1 in enumerate(self.LW_base_net.features):
                x2 = feat1(x2)
                if index1 == 13:
                    LW_features_46x46 = x2
                if index1 == 18:
                    LW_features_23x23 = x2





        RGB_features_9x9, RGB_features_5x5, RGB_features_3x3= self.RGB_aux_network.forward(RGB_features_23x23)#,RGB_features_2x2,RGB_features_1x1
        LW_features_9x9, LW_features_5x5, LW_features_3x3= self.LW_aux_network.forward(LW_features_23x23)#,LW_features_2x2,LW_features_1x1
        FUSION_refine_46x46, FUSION_refine_23x23,FUSION_refine_9x9, FUSION_refine_5x5, FUSION_refine_3x3= self.FUSION_DeConvolutions(RGB_features_46x46,RGB_features_23x23, RGB_features_9x9, RGB_features_5x5, RGB_features_3x3, LW_features_46x46, LW_features_23x23,LW_features_9x9, LW_features_5x5, LW_features_3x3)

        FUSION_locs, FUSION_class_scores = self.FUSION_prediction_network.forward(FUSION_refine_46x46 ,FUSION_refine_23x23 ,FUSION_refine_9x9 ,FUSION_refine_5x5 , FUSION_refine_3x3)#, FUSION_refine_2x2, FUSION_refine_1x1
