...
def create_data_lists(voc07_path, voc12_path, output_folder):
    ...
    # Save to file
    with open(os.path.join(output_folder, 'TRAIN_LW_images.json'), 'w') as j:
        json.dump(train_images, j)
    with open(os.path.join(output_folder, 'TRAIN_LW_objects.json'), 'w') as j:
    ...
    # Save to file
    with open(os.path.join(output_folder, 'TEST_LW_images.json'), 'w') as j:
        json.dump(test_images, j)
    with open(os.path.join(output_folder, 'TEST_LW_objects.json'), 'w') as j:
...
def transform(image, boxes, labels, difficulties, split):
    ...
    """
    assert split in {'TRAIN', 'TEST','TRAIN_LW', 'TEST_LW'}
    ...
    if (split == 'TRAIN' or split == 'TRAIN_LW'):
...
def save_checkpoint(epoch, epochs_since_improvement, model, optimizer, loss, best_loss, is_best):
    ...
    filename = 'voc_checkpoint_ssd300_new_lw.pth.tar'
