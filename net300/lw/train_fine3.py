...
from loss3 import MultiBoxLoss
# from model import SSD300, MultiBoxLoss
from datasets import KAISTdataset
from utils3 import *
from rgb_model3 import SSD_RGB
import argparse
import math
import warnings
...
def train(train_loader, model, criterion, optimizer, epoch, grad_clip):
    ...
    for i, (images, boxes, labels, _) in enumerate(train_loader):
        ...
        # Forward prop.
        predicted_locs1, predicted_scores1,predicted_locs2, predicted_scores2= model(images)  # (N, 8732, 4), (N, 8732, n_classes)
        ...
        predicted_locs1 = predicted_locs1.to(device)
        predicted_scores1 = predicted_scores1.to(device)
        ...
        loss1,loss2 = criterion(predicted_locs1, predicted_scores1,predicted_locs2, predicted_scores2, boxes, labels)  # scalar
        if (math.isnan(loss1)):
            print("Hi")
            continue
        ...
        if (loss1 != 0 ):#& (math.isnan(loss)==False)

            optimizer.zero_grad()

            loss1.backward(retain_graph=True)

        if (loss2 != 0 ):#& (math.isnan(loss)==False)
        ...
        if i % print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'trainLoss {loss.val:.4f} ({loss.avg:.4f})\t'.format(epoch, i, len(train_loader),
                                                                       ...
                                                                       data_time=data_time, loss=losses2))
    del predicted_locs1, predicted_scores1,predicted_locs2, predicted_scores2, images, boxes, labels  # free some memory since their histories may be stored
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (images, boxes, labels, difficulties) in enumerate(val_loader):
            ...
            predicted_locs2 = predicted_locs2.to(device)
            predicted_scores2 = predicted_scores2.to(device)
            loss1,loss2 = criterion(predicted_locs1, predicted_scores1, predicted_locs2, predicted_scores2, boxes, labels)
            # if (math.isnan(loss)):
            #     continue
            # if (math.isinf(loss)):
            #     continue
...
def main():
    ...
    config_file_path ="/home/fereshteh/codergb_new/config.json"
    ...
    # checkpoint = None
    # checkpoint = torch.load('./BEST_new_fine_checkpoint_ssd3001_9.pth.tar')
