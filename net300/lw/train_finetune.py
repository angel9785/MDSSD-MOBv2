...
from utils import *
from lw_model513 import SSD_LW
import argparse
import math
import warnings
...
def main():
    ...
    # Learning parameters
    # checkpoint = 'BEST_voc_checkpoint_ssd300.pth.tar'#None  # path to model checkpoint, None if none
    # checkpoint = None
    # checkpoint = torch.load('./new_fine_checkpoint_ssd300_lw.pth.tar')
    ...
    #         # print(param_name)
    #         # if (param_name=='RGB_prediction_network.cl_conv17_2.bias'):
    #         if (param_name == "extras.0.0.weight" or "extras.0.0.bias" or "extras.0.2.weight" or "extras.0.2.bias" or "extras.1.0.weight" or "extras.1.0.bias" or "extras.1.2.weight" or "extras.1.2.bias"):
    #             param.copy_(state_dict['RGB_aux_network.' + param_name])

        # for param_name, param in model.LW_DeConvolutions.named_parameters():
        #     # print(param_name)
        #     # if (param_name=='RGB_prediction_network.cl_conv17_2.bias'):
        #     param.copy_(state_dict['RGB_DeConvolutions.'+param_name])
    ...
    sanitest_loader = torch.utils.data.DataLoader(sanitest_dataset, batch_size=batch_size, shuffle=True,
                                             collate_fn=sanitest_dataset.collate_fn, num_workers=workers,
                                             pin_memory=True)
