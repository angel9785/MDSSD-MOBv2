...
class KAISTdataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.split in {'SANITRAIN', 'SANITRAIN1', 'SANITRAIN2'}:
            if i < 7601:
                rgb1, image_rgb, image_lw, boxes, labels, difficulties = transform1(m, image_rgb, image_lw, boxes,
                                                                                    labels,
                                                                                    difficulties, split=self.split)
                return rgb1, image_rgb, image_lw, boxes, labels, difficulties
            if 7600 < i < 15204:
                rgb1, image_rgb, image_lw, boxes, labels, difficulties = transform(m, image_rgb, image_lw, boxes,
                                                                                   labels,
                                                                                   difficulties, split=self.split)
                return rgb1, image_rgb, image_lw, boxes, labels, difficulties
            if 15203 < i < 22804:
                rgb1, image_rgb, image_lw, boxes, labels, difficulties = transform3(m, image_rgb, image_lw, boxes,
                                                                                    labels,
                                                                                    difficulties, split=self.split)
                return rgb1, image_rgb, image_lw, boxes, labels, difficulties
            if 22803 < i < 30404:
                rgb1, image_rgb, image_lw, boxes, labels, difficulties = transform4(m, image_rgb, image_lw, boxes,
                                                                                    labels,
                                                                                    difficulties, split=self.split)
                return rgb1, image_rgb, image_lw, boxes, labels, difficulties
