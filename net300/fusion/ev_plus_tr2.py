...
def evaluate(test_loader, model):
    ...
    with torch.no_grad():
        ...
        for i, (ir,images_rgb,images_lw, boxes1, labels, difficulties) in enumerate(tqdm(test_loader, desc='Evaluating')):
            ...
            # (success, boxes) = trackers.update(bir1)
            # ##t2=time.time()
            # ##t=t+(t2-t1)
            # ##print(t2-t1)
            # for box in boxes:
            ...
            if k%4==0:
                ...
                det_boxes_batch=torch.tensor([[]])
                det_labels_batch =torch.tensor([[]])
                det_scores_batch=torch.tensor([[]])
            k=k+1
        ...
        for i in range(bir.__len__()):
            ...
            # print(b[0].shape)
            # track_boxes = []
            # rgb = cv2.imread("/home/fereshteh/result_tr/"+str(i)+".png")
            # bir1=cv2.imread("/home/fereshteh/result_tr/"+str(i-1)+".png")
            ...
            trackers= calculate_mAP_tr(trackers,h1,b,det_boxes1[i], det_labels1[i], det_scores1[i], true_boxes[i], true_labels[i], true_difficulties[i],track_boxes)
            if m%4==0:
                trackers = cv2.MultiTracker_create()
            m=m+1
            h1=h1+1
