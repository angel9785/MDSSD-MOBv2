...
def train(train_loader, model, criterion, optimizer, epoch, grad_clip):
    ...
    # Batches
    for i, (images_rgb,images_lw, boxes, labels, _) in enumerate(train_loader):
        # if (i==8000):
        ...
        # predicted_scores=predicted_scores+rgb_scores+lw_scores
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        # Batches
        for i, (images_rgb,images_lw, boxes, labels, difficulties) in enumerate(val_loader):
...
def main():
    ...
    # Learning parameters
    # checkpoint = 'BEST_new_second_fine_checkpoint_ssd300_fusion6.pth.tar'#None  # path to model checkpoint, None if none
    # checkpoint = None
    # checkpoint = torch.load('./BEST_score_second_fine_checkpoint_ssd300_fusion4.pth.tar')
