...
class SSD_FUSION(nn.Module):
    def __init__(self, num_classes, backbone_network):
        ...
        # self.conv19 = nn.Conv2d(288, 96, 1, 1, 0)
        # self.conv10 = nn.Conv2d(3840, 1280, 1, 1, 0)
        # self.conv5 = nn.Conv2d(1536, 512, 1, 1, 0)
        # self.conv3 = nn.Conv2d(768, 256, 1, 1, 0)
        # self.conv2 = nn.Conv2d(768, 256, 1, 1, 0)
        # self.conv1 = nn.Conv2d(768, 256, 1, 1, 0)
        self.conv19 = nn.Sequential(
            nn.Conv2d(288, 96, 1, 1, 0),  # ,bias=False
            ReLU())
        self.conv10 = nn.Sequential(
            nn.Conv2d(3840, 1280, 1, 1, 0),
            ReLU())
        self.conv5 = nn.Sequential(
            nn.Conv2d(1536, 512, 1, 1, 0),
            ReLU())
    ...
    def forward(self, x1, x2):
        ...
        #     start_layer_index = end_layer_index
        #     if flag == 0:
        #         RGB_features_19x19 = layer_output1
        #         flag += 1
        #     elif flag == 1:
        #         RGB_features_10x10 = layer_output1
        # source_layer_indexes2 = [
        #     12,
        #     14, ]
        # start_layer_index2 = 0
        # flag2 = 0
        #
        # for end_layer_index2 in source_layer_indexes2:
        #     for layer2 in self.LW_base_net[start_layer_index2: end_layer_index2]:
        #         x2 = layer2(x2)
        #     layer_output2 = x2
        #     start_layer_index2 = end_layer_index2
        #     if flag2 == 0:
        #         LW_features_19x19 = layer_output2
        #         flag2 += 1
        #     elif flag2 == 1:
        #         LW_features_10x10 = layer_output2
        ...
        #         if index == 18:
        #             RGB_features_10x10 = x
        # RGB_features_10x10 = RGB_features_10x10 - torch.mean(RGB_features_10x10)
        # LW_features_10x10 = LW_features_10x10 - torch.mean(LW_features_10x10)
        features_corr_10x10 = torch.sum(RGB_features_10x10.unsqueeze(0) * LW_features_10x10.unsqueeze(0),dim=0) / (torch.sqrt(torch.sum(RGB_features_10x10.unsqueeze(0) ** 2,dim=0)) * torch.sqrt(torch.sum(LW_features_10x10.unsqueeze(0) ** 2,dim=0)))

        # RGB_features_19x19 = RGB_features_19x19 - torch.mean(RGB_features_19x19)
        # LW_features_19x19 = LW_features_19x19 - torch.mean(LW_features_19x19)
        features_corr_19x19 = torch.sum(RGB_features_19x19.unsqueeze(0) * LW_features_19x19.unsqueeze(0),dim=0) / (
                    torch.sqrt(torch.sum(RGB_features_19x19.unsqueeze(0) ** 2,dim=0)) * torch.sqrt(torch.sum(LW_features_19x19.unsqueeze(0) ** 2,dim=0)))
        # print(RGB_features_10x10.shape,LW_features_10x10.shape,features_corr_10x10.shape)
        FUSION_features_10x10 = torch.cat((RGB_features_10x10, LW_features_10x10,features_corr_10x10), dim=1)
        FUSION_features_10x10 = self.conv10(FUSION_features_10x10)
        FUSION_features_19x19 = torch.cat((RGB_features_19x19, LW_features_19x19,features_corr_19x19), dim=1)
        ...
        # RGB_features_5x5 = RGB_features_5x5 - torch.mean(RGB_features_5x5)
        # LW_features_5x5 = LW_features_5x5 - torch.mean(LW_features_5x5)
        features_corr_5x5 = torch.sum(RGB_features_5x5.unsqueeze(0) * LW_features_5x5.unsqueeze(0),dim=0) / (
                torch.sqrt(torch.sum(RGB_features_5x5.unsqueeze(0) ** 2,dim=0)) * torch.sqrt(torch.sum(LW_features_5x5.unsqueeze(0) ** 2,dim=0)))

        # RGB_features_3x3 = RGB_features_3x3 - torch.mean(RGB_features_3x3)
        # LW_features_3x3 = LW_features_3x3 - torch.mean(LW_features_3x3)
        features_corr_3x3 = torch.sum(RGB_features_3x3.unsqueeze(0) * LW_features_3x3.unsqueeze(0),dim=0) / (
                torch.sqrt(torch.sum(RGB_features_3x3.unsqueeze(0) ** 2,dim=0)) * torch.sqrt(torch.sum(LW_features_3x3.unsqueeze(0) ** 2,dim=0)))

        # RGB_features_2x2 = RGB_features_2x2 - torch.mean(RGB_features_2x2)
        # LW_features_2x2 = LW_features_2x2 - torch.mean(LW_features_2x2)
        features_corr_2x2 = torch.sum(RGB_features_2x2.unsqueeze(0) * LW_features_2x2.unsqueeze(0),dim=0) / (
                torch.sqrt(torch.sum(RGB_features_2x2.unsqueeze(0) ** 2,dim=0)) * torch.sqrt(torch.sum(LW_features_2x2.unsqueeze(0) ** 2,dim=0)))

        # RGB_features_1x1 = RGB_features_1x1 - torch.mean(RGB_features_1x1)
        # LW_features_1x1 = LW_features_1x1 - torch.mean(LW_features_1x1)
        features_corr_1x1 = torch.sum(RGB_features_1x1.unsqueeze(0) * LW_features_1x1.unsqueeze(0),dim=0) / (
                torch.sqrt(torch.sum(RGB_features_1x1.unsqueeze(0) ** 2,dim=0)) * torch.sqrt(torch.sum(LW_features_1x1.unsqueeze(0) ** 2,dim=0)))

        FUSION_features_5x5 = torch.cat((RGB_features_5x5, LW_features_5x5,features_corr_5x5), dim=1)
        FUSION_features_5x5 = self.conv5(FUSION_features_5x5)
        FUSION_features_3x3 = torch.cat((RGB_features_3x3, LW_features_3x3,features_corr_3x3), dim=1)
        FUSION_features_3x3 = self.conv3(FUSION_features_3x3)
        FUSION_features_2x2 = torch.cat((RGB_features_2x2, LW_features_2x2,features_corr_2x2), dim=1)
        FUSION_features_2x2 = self.conv2(FUSION_features_2x2)
        FUSION_features_1x1 = torch.cat((RGB_features_1x1, LW_features_1x1,features_corr_1x1), dim=1)
