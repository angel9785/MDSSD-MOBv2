...
# checkpoint = 'new4_6d_mob2_NEW_fine_checkpoint_ssd300_fusion.pth.tar'
...
def evaluate(test_loader, model):
    ...
    with torch.no_grad():
        # Batches
        for i, (ir,images_rgb,images_lw, boxes, labels, difficulties) in enumerate(tqdm(test_loader, desc='Evaluating')):
            ...
            # boxes = boxes.view(1, 10, 4)
            # labels = labels.view(1, 10, 2)
            # difficulties =difficulties.view(1, 10, 2)
        ...
        for i in range(bir.__len__()):
            # bir="/home/fereshteh/result10/"+str(i)+".png"


            g= calculate_mAP8(h,bir[i],det_boxes[i], det_labels[i], det_scores[i], true_boxes[i], true_labels[i], true_difficulties[i])
