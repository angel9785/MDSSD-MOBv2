...
class FUSION_PredictionConvolutions(nn.Module):
    ...
    def forward(self, conv11_feats, conv13_feats, conv14_2_feats, conv15_2_feats):#, conv16_2_feats, conv17_2_feats
        ...
        locs = torch.cat([l_conv11, l_conv13, l_conv14_2, l_conv15_2], dim=1)  # (N, 2268, 4), l_conv16_2, l_conv17_2
        classes_scores = torch.cat([c_conv11, c_conv13, c_conv14_2, c_conv15_2],
...
class FUSION_DeConvolutions(nn.Module):
    ...
    def __init__(self):
        ...
        # self.refine1_rgb = conv_3(256)
        # self.refine2_rgb = conv_3(256)
        # self.refine3_rgb = conv_3(256)
        self.refine5_rgb = conv_3(512)
        self.refine10_rgb = conv_3(1280)
        self.refine19_rgb = conv_3(96)

        # self.refine1_lw = conv_3(256)
        # self.refine2_lw = conv_3(256)
        # self.refine3_lw = conv_3(256)
        self.refine5_lw = conv_3(512)
        self.refine10_lw = conv_3(1280)
        self.refine19_lw = conv_3(96)
        ...
        # self.conv19 = nn.Conv2d(192, 96, 1, 1, 0)
        # self.conv10 = nn.Conv2d(2560, 1280, 1, 1, 0)
        ...
        # self.conv19_1 = nn.Sequential(
        #     nn.Conv2d(384, 96, 1, 1, 0)
        #     # ,#,bias=False
        #     #     ReLU()
        # )
        # self.conv10_1 = nn.Sequential(
        #     nn.Conv2d(5120, 1280, 1, 1, 0)
        #     # ,
        #     # ReLU()
        # )
    ...
    def forward(self,RGB_features_19x19, RGB_features_10x10, RGB_features_5x5, RGB_features_3x3, LW_features_19x19, LW_features_10x10, LW_features_5x5, LW_features_3x3):#,RGB_features_2x2,RGB_features_1x1, LW_features_2x2, LW_features_1x1

        # fusion_refine_1x1 = torch.cat((RGB_features_1x1,LW_features_1x1),dim=1)
        # FUSION_refine_1x1 = self.conv1(fusion_refine_1x1)
        # # FUSION_refine_1x1 = RGB_features_1x1+LW_features_1x1
        # FUSION_dconv_1x1 = self.dconv1(FUSION_refine_1x1)
        #
        # LW_refine1_2x2 = self.refine2_lw(LW_features_2x2)
        # RGB_refine1_2x2 = self.refine2_rgb(RGB_features_2x2)
        # cat_refine_2x2 = torch.cat((RGB_features_2x2,LW_features_2x2),dim=1)
        # fusion_refine_2x2 = self.conv2(cat_refine_2x2)
        # x2 = fusion_refine_2x2+RGB_refine1_2x2+LW_refine1_2x2+FUSION_dconv_1x1
        # FUSION_refine_2x2 = self.late2(x2)
        #
        # FUSION_dconv_2x2 = self.dconv2(FUSION_refine_2x2)

        # LW_refine1_3x3 = self.refine3_lw(LW_features_3x3)
        # RGB_refine1_3x3 = self.refine3_rgb(RGB_features_3x3)
        cat_refine_3x3 = torch.cat((RGB_features_3x3, LW_features_3x3), dim=1)
        fusion_refine_3x3 = self.conv3(cat_refine_3x3)
        # x3 = fusion_refine_3x3 + RGB_refine1_3x3 + LW_refine1_3x3 + FUSION_dconv_2x2
        # fusion_refine_3x3=RGB_features_3x3+LW_features_3x3
        ...
        # fusion_refine_5x5=RGB_features_5x5+LW_features_5x5
        x5 = fusion_refine_5x5 + FUSION_dconv_3x3+fusion_5x5#+ RGB_refine1_5x5 + LW_refine1_5x5
        # x5 = torch.cat((fusion_refine_5x5 , FUSION_dconv_3x3), dim=1)
        # x5 = self.conv5_1(x5)
        FUSION_refine_5x5 = self.late5(x5)

        FUSION_dconv_5x5 = self.dconv5(FUSION_refine_5x5)

        LW_refine1_10x10 = self.refine10_lw(LW_features_10x10)
        RGB_refine1_10x10 = self.refine10_rgb(RGB_features_10x10)
        cat_10x10 = torch.cat((RGB_features_10x10, LW_features_10x10), dim=1)
        fusion_10x10 = self.conv10(cat_10x10)
        cat_refine_10x10 = torch.cat((LW_refine1_10x10, RGB_refine1_10x10), dim=1)
        fusion_refine_10x10 = self.conv10_1(cat_refine_10x10)
        # fusion_refine_10x10=RGB_features_10x10+ LW_features_10x10
        x10 = fusion_refine_10x10 + FUSION_dconv_5x5+fusion_10x10# + RGB_refine1_10x10 + LW_refine1_10x10
        # x10 = torch.cat((fusion_refine_10x10 , FUSION_dconv_5x5), dim=1)
        # x10 = self.conv10_1(x10)
        FUSION_refine_10x10 = self.late10(x10)

        FUSION_dconv_10x10 = self.dconv10(FUSION_refine_10x10)

        LW_refine1_19x19 = self.refine19_lw(LW_features_19x19)
        RGB_refine1_19x19 = self.refine19_rgb(RGB_features_19x19)
        cat_19x19 = torch.cat((RGB_features_19x19, LW_features_19x19), dim=1)
        fusion_19x19 = self.conv19(cat_19x19)
        cat_refine_19x19 = torch.cat((LW_refine1_19x19, RGB_refine1_19x19), dim=1)
        # fusion_refine_19x19 = RGB_features_19x19+ LW_features_19x19
        fusion_refine_19x19 = self.conv19_1(cat_refine_19x19)
        x19 = fusion_refine_19x19 + FUSION_dconv_10x10 +fusion_19x19#+ RGB_refine1_19x19 + LW_refine1_19x19
        # x19 = torch.cat((fusion_refine_19x19 , FUSION_dconv_10x10 ), dim=1)
        # x19 = self.conv19_1(x19)
        FUSION_refine_19x19 = self.late19(x19)
        # cat_refine_2x2 = torch.cat((LW_features_2x2, RGB_features_2x2), dim=1)
        # FUSION_refine_2x2 = self.conv2(cat_refine_2x2)
        # fusion_refine_1x1 = torch.cat((LW_features_1x1, RGB_features_1x1), dim=1)
        # FUSION_refine_1x1 = self.conv1(fusion_refine_1x1)

        return FUSION_refine_19x19 ,FUSION_refine_10x10 ,FUSION_refine_5x5 , FUSION_refine_3x3#, FUSION_refine_2x2, FUSION_refine_1x1
...
class SSD_FUSION(nn.Module):
    def __init__(self, num_classes, backbone_network):
        ...
        # for layer in self.conv19.children():
        #     if isinstance(layer, nn.Conv2d):
        #         nn.init.xavier_uniform_(layer.weight)
        #         nn.init.constant_(layer.bias, 0.)
        #
        # for layer in self.conv10.children():
        #     if isinstance(layer, nn.Conv2d):
        #         nn.init.xavier_uniform_(layer.weight)
        #         nn.init.constant_(layer.bias, 0.)
        #
        # for layer in self.conv5.children():
        #     if isinstance(layer, nn.Conv2d):
        #         nn.init.xavier_uniform_(layer.weight)
        #         nn.init.constant_(layer.bias, 0.)
        #
        # for layer in self.conv3.children():
        #     if isinstance(layer, nn.Conv2d):
        #         nn.init.xavier_uniform_(layer.weight)
        #         nn.init.constant_(layer.bias, 0.)
        #
        # for layer in self.conv2.children():
        #     if isinstance(layer, nn.Conv2d):
        #         nn.init.xavier_uniform_(layer.weight)
        #         nn.init.constant_(layer.bias, 0.)
        #
    ...
    def forward(self, x1, x2):
        ...
        if self.backbone_net == 'MobileNetV2':
            for index, feat in enumerate(self.RGB_base_net.features):
                x1 = feat(x1)
                if index == 13:
                    RGB_features_19x19 = x1
                if index == 18:
                    RGB_features_10x10 = x1

            for index1, feat1 in enumerate(self.LW_base_net.features):
                x2 = feat1(x2)
                if index1 == 13:
                    LW_features_19x19 = x2
                if index1 == 18:
                    LW_features_10x10 = x2
        ...
        RGB_features_5x5, RGB_features_3x3= self.RGB_aux_network.forward(RGB_features_10x10)
        LW_features_5x5, LW_features_3x3= self.LW_aux_network.forward(LW_features_10x10)
        FUSION_refine_19x19, FUSION_refine_10x10, FUSION_refine_5x5, FUSION_refine_3x3= self.FUSION_DeConvolutions(RGB_features_19x19, RGB_features_10x10, RGB_features_5x5, RGB_features_3x3, LW_features_19x19, LW_features_10x10, LW_features_5x5, LW_features_3x3)


        FUSION_locs, FUSION_class_scores = self.FUSION_prediction_network.forward(FUSION_refine_19x19,
                                                                                  FUSION_refine_10x10,
                                                                                  FUSION_refine_5x5,
                                                                                  FUSION_refine_3x3,
                                                                                  )
        # for m1, box in enumerate(FUSION_locs):
