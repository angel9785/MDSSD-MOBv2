...
def create_data_lists(kaist_path, output_folder):
    ...
    # # sani=os.path.join(kaist_path, 'sanitest_txt')
    # # for path in sani:
    # sanitval_rgb = '/home/fereshteh/kaist/eval_rgb'
    # # sanitrain_lw = '/home/fereshteh/kaist/person_sani_lw'
    ...
    #
    # Save to file
    with open(os.path.join(output_folder, 'SANITEST1_images.json'), 'w') as j:
...
def calculate_mAP2(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    """
    Calculate the Mean Average Precision (mAP) of detected objects.
    ...
    true_labels = torch.cat(true_labels,dim=0)  # (n_objects)
    for i in range(len(true_labels)):
        true_images.extend([i]*a.__len__())
    true_images = torch.LongTensor(true_images).to(
            device)  # (n_objects), n_objects is the total no. of objects across all images
    # true_difficulties = torch.cat(true_difficulties, dim=0)  # (n_objects)
    print(true_images.size(0), true_boxes.size(0),true_labels.size(0))
    assert true_images.size(0) == true_boxes.size(0) == true_labels.size(0)
    ...
    for c in range(1, n_classes):
        ...
        # (n_class_detections)
        a=torch.zeros(true_class_images.size(0))
        n_easy_class_objects = (1 -a ).sum().item()  # ignore difficult objects
...
def transform(image, boxes, labels, difficulties, split):
    ...
    """
    assert split in {'SANITEST',"SANITEST1",'SANITRAIN','SANIVAL'}
    ...
    new_image1, new_boxes = resize(new_image, new_boxes, dims=(300, 300))
    # if ((new_boxes[:,0]<0).sum())>0:
    #     print("jj")
    # for i in range(0, len(new_boxes)):
    #     if (new_boxes[i, 2] == 0):
...
def save_checkpoint(epoch, epochs_since_improvement, model, optimizer, loss, best_loss, is_best):
    ...
    filename = '513_mob2_fine_checkpoint_ssd_rgb.pth.tar'
...
def save_checkpoint2(epoch, epochs_since_improvement,model1,model2,jumper,optimizer2,  loss, best_loss, is_best):
    ...
    filename = 'LW_refine_fine_checkpoint_ssd300_3.pth.tar'
    torch.save(state, filename)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, 'BEST_' + filename)
...
def calculate_mAP9(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    ...
    for c in range(1, n_classes):
        ...
        n_easy_class_objects = (true_class_difficulties!=2).sum().item()  # ignore difficult objects
...
def detect_objects(d,model, priors_cxcy, predicted_locs, predicted_scores, min_score, max_overlap, top_k, n_classes):
        """
        ...
        for i in range(batch_size):
            ...
            for c in range(1, n_classes):
                ...
                class_decoded_locs = d[0][score_above_min_score]  # (n_qualified, 4)
...
def detect_objects2(model, priors_cxcy, predicted_locs, predicted_scores, min_score, max_overlap, top_k, n_classes):
    """
    Decipher the 8732 locations and class scores (output of ths SSD300) to detect objects.
    For each class, perform Non-Maximum Suppression (NMS) on boxes that are above a minimum threshold.
    :param predicted_locs: predicted locations/boxes w.r.t the 8732 prior boxes, a tensor of dimensions (N, 8732, 4)
