...
# from rgb_model513 import SSD_RGB
from mob2_rgb import SSD_RGB
...
checkpoint = './' \
             'mob2_fine_checkpoint_ssd300_rgb.pth.tar'
# checkpoint = './fine_checkpoint_ssd300_7.pth.tar'
# Load model checkpoint that is to be evaluated
checkpoint = torch.load(checkpoint)

# model = SSD_RGB(num_classes=2)
model = SSD_RGB(num_classes=2, backbone_network="MobileNetV2")
    # global model
...
def evaluate(test_loader, model):
    ...
    with torch.no_grad():
        ...
        # Calculate mAP
        APs, mAP, precision,recall,f ,n_easy_class_objects,true_positives,false_positives= calculate_mAP_kaist(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties)
        # APs, mAP, precision,recall,f ,n_easy_class_objects,true_positives,false_positives= calculate_mAP9(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties)
